...
function getLastCarInfo(inv) {
  for(let i=0;i<inv.length;i++){
    if(i === inv.length-1){
      let carMake = inv[i].car_make;
      let carModel= inv[i].car_model;
      return `This is a ${carMake} ${carModel}`
    }
  }
}
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inv,idNum) {
  for(let i=0;i<inv.length;i++){
    if(i === idNum-1){
      let carMake = inv[i].car_make;
      let carModel= inv[i].car_model;
...
function sortCarInventory(inventory) {
  let carInv = inventory.sort(function(a,b){
...
// AAAAASSSKK WHY DID THIS WORK?!?!?!! i flipped the sign
...
function carMaker(odo) {
  var carOdo={
    ...
    drive: function drive(dist){
      carOdo.odometer = carOdo.odometer+ dist;
      return carOdo.odometer;
    }
  }
  return carOdo;
}
